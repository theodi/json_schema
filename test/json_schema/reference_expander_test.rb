...
describe JsonSchema::ReferenceExpander do
  ...
  it "expands relative references" do
    file = File.join('..', 'datapackage.rb', 'datapackage', 'schemas', 'data-package.json')

    schema = JsonSchema::Parser.new.parse!(JSON.parse(File.read(file)), nil, {uri: File.dirname(file) })
    expander = JsonSchema::ReferenceExpander.new()
    expander.expand(schema)

    error_messages = expander.errors.map { |e| e.message }
